    ...
    components: {
      ...
      MuiTableCell: {
        styleOverrides: {
          ...
          head: {
            fontWeight: 600,
            backgroundColor: mode === 'dark' ? 'rgba(149, 117, 205, 0.1)' : 'rgba(149, 117, 205, 0.05)',
          },
        },
      },
      MuiTableRow: {
        styleOverrides: {
          root: {
            '&:hover': {
              backgroundColor: mode === 'dark' ? 'rgba(255, 255, 255, 0.05)' : 'rgba(149, 117, 205, 0.05)',
            },
            '&.MuiTableRow-hover:hover': {
              backgroundColor: mode === 'dark' ? 'rgba(255, 255, 255, 0.08)' : 'rgba(149, 117, 205, 0.08)',
            },
          },
        },
      },
...
const StatusChip = ({ status }) => {
  ...
  if (statusLower.includes('new') || statusLower.includes('pending')) {
    ...
    color = 'info';
  } else if (statusLower.includes('complete') || statusLower.includes('approved')) {
    color = 'success';
...
// ----- Main Enquiries Component -----
const Enquiries = () => {
  const [darkMode, setDarkMode] = useState(false);
  const customTheme = React.useMemo(() => getLavenderTheme(darkMode ? 'dark' : 'light'), [darkMode]);
  ...
  const handleChangePage = (event, newPage) => {
    setPageNo(newPage + 1); // API uses 1-based indexing
    setEnqData([]);
  ...
  const fetchDropdownData = async () => {
    try {
      // Fetch universities
      const univResponse = await ajaxCallWithHeaderOnly(
        "universities/",
        { Authorization: `Bearer ${authData.accessToken}` }
      );
      ...
      // Fetch statuses
      const statusResponse = await ajaxCallWithHeaderOnly(
        "enquiry-status/",
        { Authorization: `Bearer ${authData.accessToken}` }
      );
  ...
  }, [enqFilter, refreshNeeded, allEnq, pageNo, perPage, authData.user_type]);
  
  // Load dropdown data on initial render
  useEffect(() => {
    fetchDropdownData();
  }, []);
  
  // Load assigned users when enqData is loaded
  useEffect(() => {
    if (enqData.length && !assignUsrData.length) {
      getAssignUsrData();
    }
  }, [enqData, assignUsrData.length]);
  ...
  const getTableColumns = () => [
    {
      ...
      renderCell: (row) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          {row.application_id ? (
            <Tooltip title="Go To Dashboard" arrow TransitionComponent={Zoom}>
              <IconButton 
                size="small" 
                component={RouterLink} 
                to={`/student/${row.id}`}
            ...
            <Tooltip title="Create Application" arrow TransitionComponent={Zoom}>
              <IconButton 
                size="small" 
                component={RouterLink} 
          ...
          <Tooltip title="Edit Enquiry" arrow TransitionComponent={Zoom}>
            <IconButton 
              size="small" 
          ...
          <Tooltip title="Delete Enquiry" arrow TransitionComponent={Zoom}>
            <IconButton 
              size="small"
              onClick={() => promptDelete(row.student_name, row.id)}
              sx={{ 
          ...
          <Tooltip title="Add Comment" arrow TransitionComponent={Zoom}>
            <IconButton 
              size="small"
    ...
    },
    {
      ...
      headerName: 'Phone',
      minWidth: 150,
      hide: columnData.phone,
      renderCell: (row) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <PhoneIcon fontSize="small" sx={{ mr: 0.75, opacity: 0.7 }} />
          {row.student_phone}
        </Box>
      ),
    },
    {
      ...
      minWidth: 200,
      flex: 1,
      hide: columnData.email,
      renderCell: (row) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <EmailIcon fontSize="small" sx={{ mr: 0.75, opacity: 0.7 }} />
          {row.student_email}
        </Box>
      ),
    },
    {
      ...
      headerName: 'Current Education',
      minWidth: 170,
      hide: columnData.current_edu,
    },
    {
      field: 'country_interested',
      headerName: 'Country Interested',
      minWidth: 170,
      hide: columnData.country_interested,
      renderCell: (row) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <FlagIcon fontSize="small" sx={{ mr: 0.75, opacity: 0.7 }} />
    ...
    },
    {
      ...
      minWidth: 130,
      hide: columnData.date,
      renderCell: (row) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <CalendarTodayIcon fontSize="small" sx={{ mr: 0.75, opacity: 0.7 }} />
          {row.date_created}
        </Box>
      ),
    },
  ...
  // Animated card for showing filters that are applied
      ...
      >
        <Card elevation={0} sx={{ p: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
            ...
            {enqFilter.university_interested && (
              <Chip 
                label={`University: ${universities.find(u => u.value === enqFilter.university_interested)?.name || 'Selected'}`} 
                size="small"
                onDelete={() => filterSelectionChanged('university_interested', null)}
                sx={{ 
            ...
            {enqFilter.level_applying_for && (
              <Chip 
                label={`Level: ${levels.find(l => l.value === enqFilter.level_applying_for)?.name || 'Selected'}`} 
                size="small"
                onDelete={() => filterSelectionChanged('level_applying_for', null)}
                sx={{ 
            ...
            {enqFilter.intake_interested && (
              <Chip 
                label={`Intake: ${intakes.find(i => i.value === enqFilter.intake_interested)?.name || 'Selected'}`} 
                size="small"
                onDelete={() => filterSelectionChanged('intake_interested', null)}
                sx={{ 
            ...
            {enqFilter.enquiry_status && (
              <Chip 
                label={`Status: ${statuses.find(s => s.value === enqFilter.enquiry_status)?.name || 'Selected'}`} 
                size="small"
            ...
            {enqFilter.assigned_usr && (
              <Chip 
                label={`Assigned To: ${assignUsrData.find(u => u.value === enqFilter.assigned_usr)?.name || 'Selected'}`} 
                size="small"
    ...
    <ThemeProvider theme={customTheme}>
      ...
      <Box sx={{ 
          ...
          <CardContent sx={{ p: { xs: 2, md: 3 } }}>
            <Grid container spacing={2} alignItems="center">
                ...
                <form onSubmit={searchEnq}>
                  <TextField
                    ...
                    InputProps={{
                      ...
                      sx: { 
                        borderRadius: 3,
                        bgcolor: alpha(customTheme.palette.background.default, 0.6),
                      }
                    }}
                  />
                </form>
              ...
              <Grid item xs={12} md={6} sx={{ display: 'flex', justifyContent: {xs: 'flex-start', md: 'flex-end'}, gap: 1 }}>
                ...
                >
                  {hasActiveFilters() ? (
                    <Badge color="primary" variant="dot" sx={{ '& .MuiBadge-dot': { right: -2, top: 3 } }}>
                      Filters
                    </Badge>
                  ) : (
                    'Filters'
                ...
                <Menu
                  ...
                  PaperProps={{
                    ...
                    sx: {
                      ...
                      border: customTheme.palette.mode === 'dark' ? `1px solid ${alpha(customTheme.palette.primary.main, 0.1)}` : 'none',
                    }
                  }}
                ...
                >
                  ...
                  <FormControl fullWidth sx={{ mb: 2.5 }}>
                    ...
                    <Select
                      ...
                      label="University"
                      onChange={(e) => filterSelectionChanged('university_interested', e.target.value)}
                      sx={{ borderRadius: 3 }}
                    >
                  ...
                  <FormControl fullWidth sx={{ mb: 2.5 }}>
                    ...
                    <Select
                      ...
                      label="Level"
                      onChange={(e) => filterSelectionChanged('level_applying_for', e.target.value)}
                      sx={{ borderRadius: 3 }}
                    >
                  ...
                  <FormControl fullWidth sx={{ mb: 2.5 }}>
                    ...
                    <Select
                      ...
                      label="Intake"
                      onChange={(e) => filterSelectionChanged('intake_interested', e.target.value)}
                      sx={{ borderRadius: 3 }}
                    >
                  ...
                  <FormControl fullWidth sx={{ mb: 2.5 }}>
                    ...
                    <Select
                      ...
                      label="Status"
                      onChange={(e) => filterSelectionChanged('enquiry_status', e.target.value)}
                      sx={{ borderRadius: 3 }}
                    >
                  ...
                  <FormControl fullWidth sx={{ mb: 2.5 }}>
                    ...
                    <Select
                      ...
                      label="Assigned To"
                      onChange={(e) => filterSelectionChanged('assigned_usr', e.target.value)}
                      sx={{ borderRadius: 3 }}
                    >
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox 
                        ...
                        onChange={() => toggleColumnVisibility('country_interested')}
                        color="primary"
                      />
                    }
                    label="Country"
              ...
              <Table sx={{ minWidth: 850 }} aria-label="enquiries table">
                <TableHead>
                  <TableRow>
                    ...
                    {!columnData.current_edu && <TableCell>Current Education</TableCell>}
                    {!columnData.country_interested && <TableCell>Country</TableCell>}
                  ...
                  {enqData.length > 0 ? (
                    enqData.map((row, index) => (
                      ...
                      >
                        ...
                        {!columnData.current_edu && <TableCell>{row.current_education}</TableCell>}
                        ...
                        {!columnData.added_by && <TableCell>{row.added_by}</TableCell>}
                      ...
                      >
                        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                          ...
                          <Typography variant="h6" fontWeight="medium">
                            {isLoadingData ? 'Loading enquiries...' : 'No enquiries found'}
                          </Typography>
                          ...
                          {!isLoadingData && (hasActiveFilters() || searchText) && (
                            <Button 
                              variant="outlined" 
                              color="primary" 
        ...
        >
          ...
          <DialogContent>
            <DialogContentText>
              Are you sure you want to delete the enquiry for <strong>{deleteEntryDetails.current.name}</strong>? This action cannot be undone.
            </DialogContentText>
          </DialogContent>

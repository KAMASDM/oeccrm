...
// Chart Components
import { LineChart, Line, AreaChart, Area, PieChart, Pie, Cell, ResponsiveContainer, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, Legend, BarChart, Bar } from 'recharts';

// Animation Libraries
import { motion } from 'framer-motion';

// Simulated API helpers (replace with actual imports)
      ...
      if (endpoint.includes('enquiries')) {
        resolve({
          results: Array(5).fill().map((_, i) => ({
            ...
            enquiry_status: ['Pending', 'In Progress', 'Completed', 'On Hold', 'New'][i % 5],
            course_name: ['Data Science', 'Business Management', 'Computer Science', 'Digital Marketing', 'Graphic Design'][i % 5]
          }))
        });
        ...
        resolve({
          results: Array(5).fill().map((_, i) => ({
            ...
            status: { App_status: ['Under Review', 'Submitted', 'Approved', 'Requires Documents', 'Payment Pending'][i % 5] },
            university: ['Oxford University', 'MIT', 'Stanford', 'Harvard', 'Cambridge'][i % 5],
            program: ['Masters in Finance', 'PhD in Physics', 'Bachelor in Business', 'Masters in Engineering', 'Bachelor in Computer Science'][i % 5]
          }))
        });
        ...
        resolve(
          Array(8).fill().map((_, i) => ({
            id: i + 1,
            content_type: { app_content: i % 2 === 0 ? 'enquiry | Student Enquiry' : 'application | Student Application' },
            object_repr: `Action ${i + 1}`,
            user: { object_id: i + 100, username: ['Alex Smith', 'Taylor Jones', 'Sam Wilson', 'Jamie Lee', 'Dakota Kim'][i % 5] },
            action_flag: [1, 2, 2, 1, 3][i % 5],
            action_time: new Date(2025, 4, 16 - Math.floor(i/2)).toISOString(),
            object_id: i + 1000
          }))
        );
...
const generateMockChartData = () => ({
  enquiryTrends: Array(7).fill().map((_, i) => ({
    date: `May ${10 + i}`,
...
const ThemeToggle = ({ darkMode, setDarkMode }) => {
  return (
    <Box sx={{ 
      display: 'flex', 
      alignItems: 'center',
      background: darkMode ? 'rgba(255, 255, 255, 0.05)' : 'rgba(149, 117, 205, 0.08)',
      borderRadius: 30,
      padding: '2px 10px',
    ...
    >
      <Box
        sx={{
          ...
          backgroundImage: `radial-gradient(${alpha('#fff', 0.15)} 1px, transparent 1px)`,
...
const EnhancedKPICard = ({ title, value, icon, trend, color, sx }) => {
  ...
  const trendIcon = isPositive ? <TrendingUpIcon fontSize="small" /> : <TrendingDownIcon fontSize="small" />;
              ...
              {trend && (
                <Chip
                  ...
                  sx={{ 
                    ...
                    height: 24,
                    borderRadius: 8,
                    boxShadow: `0 4px 8px ${alpha(isPositive ? theme.palette.success.main : theme.palette.error.main, 0.15)}`,
                  }}
                />
              )}
            </Box>
...
// Quick Actions Panel with Animations
const QuickActionsPanel = () => {
  const theme = useTheme();
  const actions = [
    { name: 'New Enquiry', icon: <HelpOutlineIcon />, to: '/enquiry/create', color: theme.palette.primary.main },
    { name: 'New Application', icon: <AddCircleOutlineIcon />, to: '/application/create', color: theme.palette.secondary.main },
    { name: 'View Reports', icon: <AssessmentIcon />, to: '/reports', color: theme.palette.info.main },
    { name: 'My Tasks', icon: <AssignmentIcon />, to: '/tasks', color: theme.palette.warning.main },
  ];
            ...
            <Grid item xs={6} sm={3} key={action.name}>
              <Button
                ...
                sx={{
                  ...
                  borderRadius: 4,
                  color: 'text.primary',
                  bgcolor: alpha(action.color, theme.palette.mode === 'dark' ? 0.1 : 0.05),
                  border: `1px solid ${alpha(action.color, theme.palette.mode === 'dark' ? 0.15 : 0.1)}`,
                  transition: 'all 0.2s',
                  '&:hover': {
                    bgcolor: alpha(action.color, theme.palette.mode === 'dark' ? 0.2 : 0.1),
              ...
              >
                <Alert 
                  ...
                  sx={{ 
                    ...
                    border: `1px solid ${alpha(theme.palette.info.main, theme.palette.mode === 'dark' ? 0.15 : 0.08)}`,
...
const EnhancedActivityFeed = ({ activities, isLoading }) => {
  ...
  const filteredActivities = useMemo(() => {
    return activities.filter(activity => {
      // Filter by type
      if (filter !== 'all' && activity.content_type.app_content.split('|')[0].trim() !== filter) {
        return false;
      }
      
      // Filter by search term
      if (searchTerm && !activity.object_repr.toLowerCase().includes(searchTerm.toLowerCase()) &&
        ...
        action={
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            <Button 
              ...
              endIcon={filter !== 'all' && (
                <Badge variant="dot" color="primary" sx={{ '& .MuiBadge-badge': { right: -4, top: 4 } }} />
              )}
              variant="outlined"
              size="small"
              sx={{ 
      ...
      <CardContent sx={{ pt: 2, pb: 1.5, maxHeight: 370, overflow: 'auto' }}>
        {isLoading ? (
              ...
              >
                <ListItemButton sx={{ 
                  ...
                  <ListItemAvatar>
                      ...
                      badgeContent={
                        <Box
                          ...
                          sx={{
                            ...
                            height: 16,
                            borderRadius: '50%',
                            bgcolor: theme.palette[getStatusColor(activity.action_flag)].main,
                            border: '2px solid',
                    ...
                    >
                      <Avatar sx={{ 
                        bgcolor: alpha(theme.palette[getStatusColor(activity.action_flag)].main, 0.15), 
                        color: theme.palette[getStatusColor(activity.action_flag)].main,
                      }}>
                    ...
                    primary={
                      <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap' }}>
                        <Typography variant="subtitle2" component="span" fontWeight="medium">
                          {activity.action_flag === 1 ? 'Created' : activity.action_flag === 2 ? 'Updated' : 'Deleted'}
                        </Typography>
                        &nbsp;
                        <Typography variant="body2" component="span" color="text.secondary">
                          {activity.content_type.app_content.split('|')[0].trim()}:
                        </Typography>
                        &nbsp;
                    ...
                    secondary={
                      <Typography variant="caption" sx={{ display: 'flex', alignItems: 'center', mt: 0.5 }}>
                        ...
                        <Box component="span" sx={{ display: 'flex', alignItems: 'center' }}>
                          <CalendarTodayIcon fontSize="inherit" sx={{ mr: 0.5, fontSize: '0.875rem' }} />
                          {new Date(activity.action_time).toLocaleString(undefined, { 
                            month: 'short', 
                            day: 'numeric', 
                            hour: '2-digit', 
                            minute: '2-digit'
          ...
          <Box sx={{ 
            ...
            <Typography color="text.secondary">No matching activities</Typography>
          </Box>
        )}
      </CardContent>
      {filteredActivities.length > 0 && (
        <Box sx={{ p: 2, textAlign: 'center', borderTop: `1px solid ${alpha(theme.palette.divider, 0.5)}` }}>
          <Button 
            size="small" 
  ...
  return (
    <Card elevation={0} sx={{ height: '100%' }}>
      <CardHeader
        ...
        subheader={`${pendingTasks} pending, ${stateTasks.length - pendingTasks} completed`}
        action={
          <Tooltip 
            title="Add New Task" 
            arrow
      ...
      <CardContent sx={{ pt: 2, pb: 1, maxHeight: 320, overflow: 'auto' }}>
        {isLoading ? (
                ...
                <ListItem 
                  ...
                  secondaryAction={
                    <IconButton 
                      ...
                      sx={{ 
                        bgcolor: task.completed ? alpha(theme.palette.success.main, 0.1) : alpha(theme.palette.action.hover, 0.1),
                        '&:hover': {
                          bgcolor: task.completed ? alpha(theme.palette.success.main, 0.2) : alpha(theme.palette.action.hover, 0.2),
                        }
                      }}
                    >
                      <TaskAltIcon />
                  ...
                  sx={{ 
                    ...
                    bgcolor: task.completed 
                      ? alpha(theme.palette.success.main, theme.palette.mode === 'dark' ? 0.05 : 0.03) 
                      : isOverdue 
                    ...
                    primary={
                      ...
                      >
                        {isOverdue && !task.completed && (
                          <Tooltip title="Overdue" arrow TransitionComponent={Zoom}>
          ...
          <Box sx={{ 
            ...
            />
            <Typography color="text.secondary" fontWeight="medium">No pending tasks. Great job!</Typography>
          </Box>
        )}
      </CardContent>
      {stateTasks.length > 0 && (
...
const EnhancedRecentItemsList = ({ title, items, isLoading, itemType }) => {
  ...
  const getStatusColor = (status) => {
    ...
    if (status.includes('progress') || status.includes('submitted') || status.includes('review')) return 'info';
    if (status.includes('complete') || status.includes('approved')) return 'success';
    if (status.includes('hold') || status.includes('rejected') || status.includes('requires')) return 'error';
    return 'default';
  };
  ...
  const getStatusIcon = (status) => {
    ...
    status = status.toLowerCase();
    if (status.includes('complete') || status.includes('approved')) return <CheckCircleIcon fontSize="small" />;
    if (status.includes('progress') || status.includes('submitted') || status.includes('review')) return <LoopIcon fontSize="small" />;
    if (status.includes('hold') || status.includes('rejected')) return <PriorityHighIcon fontSize="small" />;
    return null;
  };
  ...
  const displayItems = useMemo(() => {
    ...
    return items.filter(item => {
      ...
      const status = itemType === 'Enquiry' 
        ? item.enquiry_status?.toLowerCase() 
        : item.status?.App_status?.toLowerCase();
      
      if (tabValue === 1) // Active
        return status && (status.includes('new') || status.includes('progress') || status.includes('review') || status.includes('pending'));
      if (tabValue === 2) // Completed
        return status && (status.includes('complete') || status.includes('approved'));
  ...
  return (
    <Card elevation={0} sx={{ height: '100%' }}>
      <CardHeader
        avatar={
          <Avatar sx={{ 
            bgcolor: itemType === 'Enquiry' ? theme.palette.secondary.main : theme.palette.info.main,
            boxShadow: `0 4px 8px ${alpha(
              itemType === 'Enquiry' ? theme.palette.secondary.main : theme.palette.info.main, 
              0.25
            )}`
          }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Chip
                    ...
                    sx={{ 
                      ...
                      fontWeight: 'bold',
                      fontSize: '0.7rem',
                      bgcolor: tabValue === getTabs().findIndex(t => t.label === tab.label) 
        ...
        {isLoading ? (
          ...
          <List disablePadding>
            {displayItems.map((item, index) => {
              ...
              const date = itemType === 'Enquiry'
                ? new Date(item.date_created).toLocaleDateString()
                : new Date(item.created_at).toLocaleDateString();
              
              const secondaryInfo = itemType === 'Enquiry'
                ? item.course_name
                : `${item.university || 'University'} - ${item.program || 'Program'}`;
              ...
              return (
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Chip
                        ...
                        sx={{ 
                          mr: 1.5,
                          fontWeight: 'medium',
                          boxShadow: `0 2px 4px ${alpha(theme.palette[statusColorCode].main, 0.2)}`
                        }}
                      />
                      <Tooltip title="View Details" arrow TransitionComponent={Zoom}>
                        <IconButton
                          ...
                          aria-label="details"
                          component={RouterLink}
                          to={`/${itemType.toLowerCase()}/${itemType === 'Enquiry' ? 'edit' : 'edit'}/${item.id}`}
                          size="small"
                          sx={{ 
                            bgcolor: alpha(theme.palette.primary.main, 0.05),
                            '&:hover': {
                ...
                >
                  <ListItemButton sx={{ 
                    ...
                    <ListItemText
                      primary={
                        <Typography variant="subtitle2" fontWeight="medium">{name}</Typography>
                      }
                      secondary={
                        <Typography variant="caption" sx={{ display: 'flex', flexDirection: 'column', mt: 0.5 }}>
                          <Box component="span" sx={{ mb: 0.2, display: 'flex', alignItems: 'center' }}>
                            <CalendarTodayIcon fontSize="inherit" sx={{ mr: 0.5, fontSize: '0.75rem' }} />
                            {date}
                          </Box>
          ...
          <Box sx={{ 
            ...
            <Typography color="text.secondary">
              No {tabValue === 0 ? '' : getTabs()[tabValue].label.toLowerCase()} {itemType.toLowerCase()}s found.
            </Typography>
          </Box>
        )}
      </CardContent>
            ...
            >
              ...
              <defs>
                <linearGradient id="colorConversion" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={theme.palette.info.main} stopOpacity={0.9}/>
                  <stop offset="95%" stopColor={theme.palette.info.light} stopOpacity={0.6}/>
                </linearGradient>
              </defs>
              <Bar 
                dataKey="rate" 
  ...
  return (
    <Card elevation={0} sx={{ height: '100%' }}>
      <CardHeader
        ...
        action={
          <Box sx={{ display: 'flex', gap: 0.5, bgcolor: alpha(theme.palette.background.default, 0.6), borderRadius: 3, p: 0.5 }}>
            <Button
              size="small"
...
const HomePage = () => {
  ...
  // Create theme with dynamic light/dark mode
  const customTheme = useMemo(() => getLavenderTheme(darkMode ? 'dark' : 'light'), [darkMode]);
  
  // Existing state and selectors
  const authData = useSelector((state) => state.authStore || { userName: "Demo User", user_type: "admin" });
  ...
  const fetchUserTasks = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setUserTasks([
          { id: 1, title: 'Follow up with John Doe about visa application', dueDate: '2025-05-17', completed: false },
          { id: 2, title: 'Prepare application documents for Jane Smith', dueDate: '2025-05-15', completed: true },
          { id: 3, title: 'Call University of London about admission status', dueDate: '2025-05-16', completed: false },
          { id: 4, title: 'Submit progress report for Q2', dueDate: '2025-05-20', completed: false },
        ]);
  ...
  const getEnqData = useCallback(async () => {
    ...
    try {
      const response = await ajaxCallWithHeaderOnly(
        ...
        { Authorization: `Bearer ${authData.accessToken}` }, "GET", null
      );
      if (response?.isNetwork || response?.status === 401) {
        setThrowErr({ ...response, page: "dashboard-enquiries" }); return;
      }
      if (response?.results) {
  ...
  const getAppData = useCallback(async () => {
    ...
    try {
      ...
      if (response?.isNetwork || response?.status === 401) {
        setThrowErr({ ...response, page: "dashboard-applications" }); return;
      }
      if (response?.results) {
  ...
  const getActivityData = useCallback(async () => {
    ...
    try {
      ...
      if (response?.isNetwork || response?.status === 401) {
        setThrowErr({ ...response, page: "dashboard-activity" }); return;
      }
      if (response?.length) {
  ...
  const getMsgData = useCallback(async () => {
    ...
    try {
      ...
      if (response?.isNetwork || response?.status === 401) {
        setThrowErr({ ...response, page: "dashboard-messages" }); return;
      }
      if (response?.length) {
        setMsgData(response);
      } else {
  ...
  useEffect(() => {
    ...
    // Set chart data (in real implementation, this would come from an API)
    ...
    <ThemeProvider theme={customTheme}>
      ...
      >
        ...
        <Grid container spacing={3}>
          {/* Welcome Banner */}
          <Grid item xs={12}>
            <WelcomeBanner userName={authData.userName || "User"} darkMode={darkMode} />
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <EnhancedKPICard 
              title="New Enquiries" 
              value={isKpiLoading ? <CircularProgress size={24}/> : kpiData.newEnquiriesToday} 
          ...
          <Grid item xs={12} sm={6} md={4}>
            <EnhancedKPICard 
              ...
              value={isKpiLoading ? <CircularProgress size={24}/> : kpiData.applicationsInProgress} 
              icon={<LoopIcon />}
          ...
          <Grid item xs={12} sm={6} md={4}>
            <EnhancedKPICard 
              ...
              value={isKpiLoading ? <CircularProgress size={24}/> : kpiData.tasksDueToday} 
              icon={<AssignmentIcon />} 
          ...
          {/* Quick Actions & Broadcast Messages Row */}
          <Grid item xs={12} md={authData.user_type === "staff" ? 12 : 8}>
            {authData.user_type !== "staff" && <QuickActionsPanel />}
          </Grid>
          {authData.user_type !== "staff" && (
            <Grid item xs={12} md={4}>
              <EnhancedBroadcastMessages messages={msgData} isLoading={isMsgLoadingData} />
            </Grid>
          )}

          {/* Recent Activity & Tasks */}
          <Grid item xs={12} lg={authData.user_type === "staff" ? 12 : 8}>
            <EnhancedActivityFeed activities={activityData} isLoading={isActivityLoadingData} />
          </Grid>
          {authData.user_type !== "staff" && (
